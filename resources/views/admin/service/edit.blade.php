...
<body>
    {{-- Navbar --}}
    <form action="{{ route('admin.service.update', $service->id) }}" method="post" id="serviceForm">
        @method('put')
        @csrf
                ...
                <ul class="navbar-nav me-auto">
                    <li class="nav-item">
                        <input type="text" class="form-control navbar" name="title" id="title"
                            placeholder="Judul Artikel" value="{{ $service->title }}" required>
                        @if ($errors->has('title'))
                    ...
                    <li class="nav-item">
                        <select class="form-control" name="status" id="status">
                            <option value="0" {{ $service->status ? '' : ' selected' }}>Aktif</option>
                            <option value="1" {{ $service->status ? ' selected' : '' }}>Tidak Aktif</option>
                        </select>
                    </li>
                </ul>
